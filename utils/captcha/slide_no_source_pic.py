...
def auto_login_base64_pic(name, url, account_input_xpath, password_input_xpath, login_btn_xpath, username, password, is_wait,
...
def auto_login_url_pic(name, url, account_input_xpath, password_input_xpath, login_btn_xpath, username, password, is_wait,
               wait_ele_xpath, big_img, small_img, scale):
    """图片是url形式的"""
    # Options类实例化
    chrome_options = Options()
    ua = UserAgent()
    ...
    for _ in range(20):
        # 查看图片路径是否存在
        big_img_path = f'resources/big_{name}.png'
        small_img_path = f'resources/small_{name}.png'
        if not os.path.exists(big_img_path) or not os.path.exists(small_img_path):
            print('无法获取到验证码图片...')
            break

        # 滑块移动距离
        x = find_pic(big_img_path, small_img_path)
        print(x)
        # 真实尺寸和渲染尺寸有偏差：在浏览器通过调试工具elements将render size / Intrinsic size
        x = (x * (242 / 360)) * scale
        print(f'实际需要移动:{x}')
